...
public:
    char findKthBit(int n, int k)
    {
        ...
        {
            int symmetric_k = len - k + 1;
            char bit = findKthBit(n - 1, symmetric_k);
            return (bit == '0') ? '1' : '0';
        }
    }
};
...
public:
    ...
    int cal(vector<int> &nums, int gcd1, int gcd2, int idx)
    {
        ...
        int take1 = cal(nums, std::gcd(gcd1, nums[idx]), gcd2, idx + 1);

        int take2 = cal(nums, gcd1, std::gcd(gcd2, nums[idx]), idx + 1);

        int dtake = cal(nums, gcd1, gcd2, idx + 1);

        return dp[idx][gcd1][gcd2] = ((take1 + take2) % MOD + dtake) % MOD;
    }
